  ...
  Future<List<Products>> getAllProducts() async {
    final db = await instance.database;
    final List<Map<String, dynamic>> products = await db.query(tableInventory);
  ...
  Future<void> updateProductQuantity(int productId, int newQuantity) async {
    ...
    Future<void> updateProductCost(int productId, double newCost) async {
    final db = await instance.database;
    await db.update(
      tableInventory,
      {ProductFields.cost: newCost},
      where: '${ProductFields.id} = ?',
      whereArgs: [productId],
    );
  }
...
Future<List<Products>> searchProducts(String searchText) async {
  final db = await instance.database;
  final List<Map<String, dynamic>> products = await db.query(
    tableInventory,
